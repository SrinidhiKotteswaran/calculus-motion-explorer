  ...
  return <div className="min-h-screen" style={{ background: 'var(--bg)' }}>
    <header className="sticky top-0 z-20 border-b border-default" style={{ background: 'color-mix(in srgb, var(--bg) 92%, transparent)', backdropFilter: 'blur(12px)' }}>
      <div className="max-w-[1400px] mx-auto px-5 h-16 flex items-center justify-between gap-5">
        <button onClick={()=>setMode('explore')} className="flex items-center gap-3 text-left">
        ...
        <nav className="hidden lg:flex items-center gap-1">{nav.map(item=>{const Icon=item.icon; return <button key={item.id} onClick={()=>setMode(item.id)} className={`flex items-center gap-2 px-3 py-2 rounded-md text-sm transition-colors ${mode===item.id?'bg-elevated text-[var(--text)]':'text-secondary hover:text-[var(--text)]'}`}><Icon size={15}/>{item.label}</button>})}</nav>
        <ThemeToggle />
      </div>
      <div className="lg:hidden overflow-x-auto border-t border-default"><div className="flex min-w-max px-4 py-2 gap-1">{nav.map(item=><button key={item.id} onClick={()=>setMode(item.id)} className={`px-3 py-1.5 text-xs rounded-md ${mode===item.id?'bg-elevated text-[var(--text)]':'text-secondary'}`}>{item.label}</button>)}</div></div>
    </header>
