...
export function Panel({ children, className = '' }: { children: ReactNode; className?: string }) {
  return (
    <div className={`bg-card border border-default rounded-lg p-5 ${className}`}>
      {children}
    </div>
  );
}
...
export function SectionTitle({ children, subtitle }: { children: ReactNode; subtitle?: string }) {
  return (
    <div className="mb-4">
      <h2 className="text-lg font-semibold tracking-tight" style={{ color: 'var(--text)' }}>{children}</h2>
...
export function Button({
  ...
  const variants = {
    default: active
      ...
      : 'bg-elevated border border-default text-secondary hover:text-[var(--text)] hover:border-light',
        ...
        {[0.25, 0.5, 1, 2, 4].map((s) => (
          <button
            ...
            className={`px-2 py-1 text-xs rounded transition-colors ${
              speed === s ? 'bg-[var(--accent)] text-white' : 'text-secondary hover:text-[var(--text)]'
            }`}
          >
            {s}x
          </button>
...
export function ThemeToggle() {
  ...
  return (
    <button
      onClick={toggle}
      className="p-2 rounded-md text-secondary hover:text-[var(--text)] transition-colors"
    ...
    <div>
      {label && (
        <div className="flex justify-between items-center mb-1.5">
          ...
          {valueDisplay && <span className="text-xs text-secondary font-mono">{valueDisplay}</span>}
        </div>
      )}
...
export function StatCard({ label, value, unit, color }: { label: string; value: string; unit?: string; color?: string }) {
  return (
    <div className="bg-elevated border border-default rounded-md px-3 py-2.5">
      <div className="text-xs text-secondary uppercase tracking-wider mb-1">{label}</div>
      <div className="text-lg font-semibold font-mono" style={{ color: color || 'var(--text)' }}>
        {value}
        {unit && <span className="text-sm text-secondary ml-1 font-normal">{unit}</span>}
      </div>
    </div>
  );
}
...
export function FunctionInput({
  ...
  return (
    <div>
      <Label>Function s(t)</Label>
      <div className="flex items-center gap-2">
        <span className="text-secondary font-mono text-sm whitespace-nowrap">s(t) =</span>
        <input type="text" value={value} onChange={(e) => onChange(e.target.value)} placeholder="e.g. t^3 - 4*t^2 + 2*t" spellCheck={false} />
      </div>
        ...
        <div className="flex flex-wrap gap-1.5 mt-2">
          {presets.map((p) => (
            <button
              key={p}
              onClick={() => onPreset?.(p)}
              className="text-xs px-2 py-1 rounded border border-default text-secondary hover:text-[var(--text)] hover:border-light transition-colors font-mono"
            >
              {p}
            </button>
          ))}
